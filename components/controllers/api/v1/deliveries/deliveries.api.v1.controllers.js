...
router
  ...
  .get(
    (req, res, next) =>
      handler(getValidator, req, res, next, validations["mainGet"]),
    (req, res, next) => responses(deliveriesCommonServices.find, req, res, next)
  )
  .post(
    (req, res, next) =>
      handler(validator, req, res, next, validations["mainPost"]),
    (req, res, next) =>
      responses(deliveriesCommonServices.create, req, res, next)
  );

router.route("/:id").get(
  (req, res, next) =>
    handler(getValidator, req, res, next, validations["getOne"]),
  (req, res, next) =>
    responses(deliveriesCommonServices.findOne, req, res, next)
);

router.route("/search").post(
  (req, res, next) =>
    handler(getValidator, req, res, next, validations["search"]),
